...
if ($cart->num_rows() > 0) {
    foreach ($cart->result() as $c => $data) { ?>
        <tr>
            ...
            <td class="text-center"><?= $data->fn_diskon ?></td>
            <td class="text-center"><?= $data->fn_pajak ?> %</td>
            <td class="text-center"><?= ($data->harga_beli) + ($data->harga_beli) * $data->fn_pajak / 100 ?></td>
            ...
            <td class="text-center" widht="160px">
                ...
                <button id="del_cart" data-cartid="<?= $data->fs_id_cart_pembelian ?>" class="btn btn-sm btn-danger">
