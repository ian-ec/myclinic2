        ...
        <div class="card">
            <div class="card-body">
                <div class="form-group row mb-4">
                    <label for="horizontal-firstname-input" class="col-sm-3 col-form-label">Kode Tarif</label>
                    <div class="col-sm-9">
                        <input type="hidden" id="fs_id_tarif" name="fs_id_tarif" value="">
                ...
                <div class="form-group row mb-4">
                    ...
                    <div class="col-sm-9">
                        <input type="text" id="fs_nm_tarif" value="" class="form-control" required>
                    </div>
                </div>
                <div class="form-group row mb-4">
                    ...
                    <div class="col-sm-9">
                        <select id="fs_id_rekapcetak" name="fs_id_rekapcetak" class="form-control select2" required>
                            ...
                            <?php foreach ($rekapcetak->result() as $key => $rc) { ?>
                                <option value="<?= $rc->fs_id_rekapcetak ?>"><?= $rc->fs_nm_rekapcetak ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                <div class="form-group row mb-4">
                    ...
                    <div class="col-sm-9">
                        <input type="number" id="fn_nilai_tarif" value="" class="form-control" min="0" required>
                    </div>
                </div>
                <div class="form-group row mb-4 float-right">
                    <div class="col-sm-12">
                        <button id="simpan_tarif" class="btn btn-info btn-flat">
                        ...
                        <div class="card-body table-responsive">
                                ...
                                <tr>
                                    <td>
                                        <div class="form-group row mb-4 input-group">
                                            ...
                                            <div class="col-sm-5 form-group input-group">
                                                <button type="button" id="add_cart_layanan" class="btn btn-primary btn-sm">
                            ...
                            <table class="table table-bordered table-sm table-striped" style="width: 100%;">
                                ...
                                <tbody id="cart_table_layanan">
                                    <?php $this->view('master_data_billing/tarif/tarif_cart_layanan') ?>
                                </tbody>
                            </table>
                        </div>
                    </div>
...
<script>
    ...
    $(document).on('click', '#add_cart_layanan', function() {
        ...
        if (fs_id_layanan == '') {
            ...
            $.ajax({
                ...
                data: {
                    'add_cart_layanan': true,
                    'fs_id_layanan': fs_id_layanan
                },
                dataType: 'json',
                success: function(result) {
                    if (result.success == true) {
                        $('#cart_table_layanan').load('<?= site_url('tarif/cart_data_layanan') ?>')
                        $('#fs_id_layanan').val('')
                        $('#fs_kode_layanan').val('')
                        $('#fs_nm_layanan').val('')
                        $('#fs_nm_layanan').focus()
    ...
    $(document).on('click', '#del_cart_layanan', function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var cart_id_layanan = $(this).data('cartidly')
                $.ajax({
                    type: 'POST',
                    url: '<?= site_url('tarif/process') ?>',
                    data: {
                        'del_cart_layanan': true,
                        'cart_id_layanan': cart_id_layanan
                    },
                    dataType: 'json',
                    success: function(result) {
                        if (result.success == true) {
    ...
    $(document).on('click', '#simpan_tarif', function() {
